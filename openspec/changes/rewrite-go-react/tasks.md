...
- [ ] 2.1.2 定义环境变量 schema（与 .env.example 1:1 对应）
  ...
  - [ ] 2.1.2c 熔断器配置（ENABLE_CIRCUIT_BREAKER_ON_NETWORK_ERRORS、MAX_RETRY_ATTEMPTS_DEFAULT）
  - [ ] 2.1.2d 智能探测配置（ENABLE_SMART_PROBING、PROBE_INTERVAL_MS、PROBE_TIMEOUT_MS）
  - [ ] 2.1.2e Cookie 安全配置（ENABLE_SECURE_COOKIES）
  - [ ] 2.1.2f API 测试配置（API_TEST_TIMEOUT_MS）
  - [ ] 2.1.2g 多供应商类型配置（ENABLE_MULTI_PROVIDER_TYPES）
- [ ] 2.1.3 实现配置验证
...
- [ ] 2.4.1 配置 Zap 日志
- [ ] 2.4.2 实现结构化日志格式
- [ ] 2.4.3 实现日志级别动态调整（运行时切换）
- [ ] 2.4.4 实现日志输出配置（stdout/file）
